...
export default function SubscriptionModal({ subscription, categories, people: initialPeople, onPeopleAdded, onClose, onSaved }) {
  ...
  const [form, setForm] = useState({
    ...
    ...(subscription ? {
      ...
      assigned_user_id: subscription.owner_id || "",
  ...
  // Load users list for top-role assignment
  useEffect(() => {
    if (isTopRole) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        next_due_date:
          form.billing_cycle === "One Time"
            ? form.next_due_date || new Date().toISOString().slice(0, 10)
            : form.next_due_date,
        assigned_user_id: isTopRole && form.assigned_user_id ? form.assigned_user_id : undefined,
      };
      if (isEdit) {
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4" data-testid="subscription-modal">
      <div className="bg-card border border-border rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex items-center justify-between px-6 py-4 border-b border-border sticky top-0 bg-card z-10">
          ...
          <button onClick={onClose} className="text-muted-foreground hover:text-foreground transition-colors" data-testid="modal-close-button">
            <X size={18} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div className="sm:col-span-2">
              <Field label="Subscription Name" required>
                <input
                  ...
                  className={inpCls}
                  placeholder="e.g. Netflix, AWS, Laptop lease"
                  required
            ...
            <Field label="Billing Cycle" required>
              ...
              {isOneTime && (
                <p className="text-xs text-blue-600 dark:text-blue-400 mt-1">
                  One-time cost won't appear in monthly/annual totals
                </p>
              )}
            </Field>
            ...
            {/* Assign to User — only for top roles */}
            {isTopRole && (
              <Field label="Assign to User">
                <select
                  ...
                  className={inpCls}
                  data-testid="sub-assigned-user-select"
                >
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Subscription will be created under the selected user's account.
                </p>
              </Field>
            )}
            ...
            <div className="sm:col-span-2">
              <Field label="Management Link">
                <input
                  value={form.management_link}
                  onChange={(e) => set("management_link", e.target.value)}
                  className={inpCls}
                  placeholder="https://..."
                  data-testid="sub-link-input"
                />
              </Field>
            </div>
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="submit"
              disabled={loading}
              className="flex-1 px-4 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg text-sm font-semibold transition-colors disabled:opacity-50"
